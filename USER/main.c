		...
		if((BootStatusFlag&UpDataA_Flag)){
			...
			if(OTA_Info.FireLen[UpDataA.W25Q64_BlockNumber] % 4 == 0){
				...
				for(i=0;i<OTA_Info.FireLen[UpDataA.W25Q64_BlockNumber]/1024;i++){
					...
					FLASH_Write(STM32_A_Start_Addr+i*1024,(uint32_t *)UpDataA.UpDataA_Buf,1024);
				}
				// 处理不足1KB的剩余数据
				if(OTA_Info.FireLen[UpDataA.W25Q64_BlockNumber] % 4 != 0){
					...
					FLASH_Write(STM32_A_Start_Addr+i*1024,(uint32_t *)UpDataA.UpDataA_Buf,OTA_Info.FireLen[UpDataA.W25Q64_BlockNumber] % 4);
				}
				// 如果是第一个块，清除OTA标志
