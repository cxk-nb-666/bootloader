...
{	
	...
	while (1)
	{
		...
		{
			U1_printf("长度%d", OTA_Info.FireLen[UpDataA.W25Q64_BlockNumber+1]);
			// 检查固件长度是否为4的倍数（字对齐）
			if (OTA_Info.FireLen[UpDataA.W25Q64_BlockNumber+1] % 4 == 0)
			{
				// 擦除应用程序区域的Flash
				...
				{
					W25Q128_ReadData(i * 1024 + UpDataA.W25Q64_BlockNumber * 64 * 1024, UpDataA.UpDataA_Buf, OTA_Info.FireLen[UpDataA.W25Q64_BlockNumber] % 1024);
